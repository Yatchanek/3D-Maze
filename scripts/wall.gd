...
func _ready() -> void:
	if type == Type.TOP:
		wall_body.mesh.size = Vector3(Globals.HEX_SIZE * 0.5, 0.5, Globals.WALL_WIDTH)
		position.y = Globals.HEX_HEIGHT - 0.25
		collision_shape.set_deferred("disabled", true)
		outline.queue_free()
	else:
		wall_body.mesh.size = Vector3(Globals.HEX_SIZE * 0.5, Globals.HEX_HEIGHT, Globals.WALL_WIDTH)
